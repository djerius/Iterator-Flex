...
the additional L<prev|Iterator::Flex::Manual::Overview/prev> and
L<current|Iterator::Flex::Manual::Overview/current> capabilities.  We
also keep track of the array, C<$arr>, and its length C<$len>.
...
behavior.  C<next> should thus I<always> call C<signal_exhaustion> when
exhausted, even if the iterator has already signaled exhaustion.
...
we are using closed over variables to keep track of state. That means
...
At this point C<%AttrHash> is functionally complete. The only thing left unknown is
...
Now we're ready to use the C<%AttrHash> to construct an
