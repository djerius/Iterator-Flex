...
L</freeze>.  Given a serialized iterator, it reconsititue it so that
...
There are three classes of parameters used by iterators and adaptors:
...
I<Interface> parameters provide implementations of the capabilites that the
iterator provides.
...
The state is also queriable via the
...
This applies only to iterator adaptors, and indicates how the consumed iterator signals exhaustion.
...
Only for adaptors; the consumed iterator will throw on exhaustion; let that exception propagate.
...
For both iterators and adaptors, indicates that a sentinel value of
c<undef> will be returned.

=item C<< [ return => $value ] >>

For both iterators and adaptors, indicates that the specified sentinel value will be returned.

=back

=item C<exhaustion>

This applies to both iterators and  adaptors, and indicates how they signal exhaustion.
It takes the same values as L</input_exhaustion>

=item C<error>

This applies to both iterators and  adaptors, and indicates how they signal error.
It may have one of the following values:
