...
Serialize an iterator after every call to C<next>.  C<CODE> will be
passed a frozen state (generated by calling C<$iterator->freeze> via
C<$_>, with which it can do as it pleases.

This differs from L<imap> in that C<CODE> I<is> executed when
C<$iterator> returns I<undef> (that is, when C<$iterator> is
exhausted.
...
sub ifreeze (&$) {
    ...
    croak( "ifreeze requires the iterator to provide a freeze method\n" )
